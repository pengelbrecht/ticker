...
- Injected into every task's prompt template (agent-agnostic)
...
**Skip context generation if the epic has ≤1 children.** There's no amortization benefit for single-task epics—the context generation cost would exceed the savings.
...
│                                                              │
│  1. Check child count                                        │
│     └─ Epic has ≤1 children? → skip context generation       │
│                                                              │
...
│                                                              │
│  4. Run context generation agent                             │
...
### 3. System Prompt via CLI Flag
Use `claude --append-system-prompt` to inject context into the system prompt.

**Rejected:** Claude-specific flag that breaks multi-agent support. Prompt template injection is portable across all agents (Codex, Gemini, etc.) and gives full control over formatting and ordering.
...
- [prompt.go](../internal/engine/prompt.go) - Prompt template (injection point for context)
